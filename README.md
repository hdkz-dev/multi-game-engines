...
- **Zenith Robustness & High Coverage**: `core` パッケージのラインカバレッジは目標 **≥98.4%** (PR #49 時点 98.41%)。最近のネイティブ通信層追加で **84.6% (2026-05-09 計測)** へ低下しており、復元タスクが [TASKS.md](docs/TASKS.md) で進行中。ミドルウェア絶縁、循環参照保護、パケット分割耐性は実装済み。
