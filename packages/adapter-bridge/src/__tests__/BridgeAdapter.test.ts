...
import { BridgeAdapter, createBridgeAdapter } from "../BridgeAdapter.js";
...
describe("BridgeAdapter", () => {
  const config: IEngineConfig = {
    id: "test-bridge",
    adapter: "bridge",
    name: "Test Bridge Engine",
    sources: {
      main: {
        url: "https://example.com/bridge-engine.js",
        __unsafeNoSRI: true,
        type: "worker-js",
      },
    },
  ...
  it("should initialize with config metadata", () => {
    const adapter = new BridgeAdapter(config);
    expect(adapter.id).toBe("test-bridge");
    expect(adapter.name).toBe("Test Bridge Engine");
  });
  ...
  it("createBridgeAdapter factory returns a BridgeAdapter instance", () => {
    const adapter = createBridgeAdapter(config);
    expect(adapter).toBeInstanceOf(BridgeAdapter);
    expect(adapter.id).toBe("test-bridge");
  });
});

describe("BridgeAdapter — native mode (Node.js + binaryPath)", () => {
  ...
  it("should reach ready status without a loader when binaryPath is set", async () => {
    const adapter = new BridgeAdapter(nativeConfig);
    await adapter.load();
    expect(adapter.status).toBe("ready");
  });
