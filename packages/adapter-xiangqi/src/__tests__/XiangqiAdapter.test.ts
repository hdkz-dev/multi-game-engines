...
class MockWorker {
  postMessage = vi.fn((msg: unknown) => {
    if (
      ...
      setTimeout(() => {
        if (typeof this.onmessage === "function") {
          this.onmessage({ data: "id name TestXiangqi" } as MessageEvent);
          this.onmessage({ data: "ucciok" } as MessageEvent);
        }
      }, 0);
    }
  });
...
describe("XiangqiAdapter", () => {
  ...
  it("should load successfully when source has mountPath (injectResources path)", async () => {
    ...
    const adapter = new XiangqiAdapter(configWithMount);
    await adapter.load(loaderWithBook as unknown as IEngineLoader);
    expect(adapter.status).toBe("ready");
  });

  it("should change status to ready on successful load with ucciok handshake", async () => {
    const adapter = new XiangqiAdapter(config);
