...
/**
 * Component E2E tests for PVList (React).
 *
 ...
 */
test.describe("PVList", () => {
  const makeMove = (s: string): Move => s as unknown as Move;

  test("renders empty state when no PVs provided", async ({ mount }) => {
    const component = await mount(<PVList pvs={[] as PrincipalVariation[]} />);

    await expect(component).toBeVisible();
    // Default locale is Japanese: "探索中..." — match any non-empty searching text
    await expect(component).toContainText(/.+/);
    // Ensure no PV rows are rendered
    await expect(component.locator(".flex-col.p-3")).toHaveCount(0);
  });
  ...
  test("renders a single PV with centipawn score", async ({ mount }) => {
    ...
    const component = await mount(<PVList pvs={pvs} />);
  ...
  test("calls onMoveClick when a move is clicked", async ({ mount }) => {
