...
import { IEngineAdapter, IBaseSearchOptions, IBaseSearchInfo, IBaseSearchResult } from "../types.js";
...
describe("Loading Strategies", () => {
  const createMockAdapter = () => ({
    ...
    searchRaw: vi.fn().mockReturnValue({
      info: (async function* () { yield { raw: "info" } as IBaseSearchInfo; })(),
  ...
  it("'manual' 戦略の場合、未ロードで search() を呼ぶとエラーになること", async () => {
    ...
    await expect(facade.search({})).rejects.toThrow(/Engine is not initialized/);
  });
