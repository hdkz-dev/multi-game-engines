...
import { fixture, html } from "@open-wc/testing-fixture";
import "../index.js";
...
describe("ShogiBoard: Absolute Boundary Keyboard Navigation", () => {
  ...
  const focusSquare = async (index: number) => {
    const square = el.shadowRoot?.querySelector(`[data-index="${index}"]`) as HTMLElement;
    square?.focus();
    // 内部状態を同期させるために直接代入（テストの安定性のため）
    (el as unknown)._focusedIndex = index;
    await el.updateComplete;
  };

  const pressKey = (key: string, ctrl = false) => {
    el.dispatchEvent(new KeyboardEvent("keydown", { key, ctrlKey: ctrl, bubbles: true, composed: true }));
  };
