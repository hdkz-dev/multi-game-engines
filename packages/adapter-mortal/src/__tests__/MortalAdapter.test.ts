...
import { MortalAdapter, createMortalAdapter } from "../MortalAdapter.js";
...
describe("MortalAdapter", () => {
  ...
  it("should call setOption BookFile via onBookLoaded after successful load", async () => {
    ...
    expect(workerInstance!.postMessage).toHaveBeenCalledWith(
      expect.objectContaining({ name: "BookFile" }),
    );
  });

  it("createMortalAdapter factory returns a MortalAdapter instance", () => {
    const adapter = createMortalAdapter(mockConfig);
    expect(adapter).toBeInstanceOf(MortalAdapter);
    expect(adapter.id).toBe("mortal");
  });
});
