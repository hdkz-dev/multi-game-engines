...
import { GNUBGAdapter, createGNUBGAdapter } from "../GNUBGAdapter.js";
...
describe("GNUBGAdapter", () => {
  ...
  it("should load successfully when source has mountPath (injectResources path)", async () => {
    ...
    const adapter = new GNUBGAdapter(configWithMount);
  ...
  it("createGNUBGAdapter factory returns a GNUBGAdapter instance", () => {
    const adapter = createGNUBGAdapter(mockConfig);
    expect(adapter).toBeInstanceOf(GNUBGAdapter);
    expect(adapter.id).toBe("gnubg");
  });
});
