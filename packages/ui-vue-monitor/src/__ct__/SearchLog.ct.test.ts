...
/**
 * Component E2E tests for SearchLog (Vue).
 *
...
test.describe("SearchLog (Vue)", () => {
  ...
  test("renders empty state when log is empty", async ({ mount }) => {
    const component = await mount(SearchLog, {
      props: { log: [] as SearchLogEntry[] },
    });
  ...
  test("renders multiple entries in table rows", async ({ mount }) => {
    ...
    const component = await mount(SearchLog, { props: { log } });
  ...
  test("renders seldepth alongside depth", async ({ mount }) => {
    ...
    const component = await mount(SearchLog, { props: { log: [entry] } });
  ...
  test("emits move-click when a PV move is clicked", async ({ mount }) => {
    const clicked: string[] = [];
    const entry = makeEntry({ pv: [makeMove("e2e4"), makeMove("e7e5")] });

    const component = await mount(SearchLog, {
      props: { log: [entry] },
      on: { "move-click": (move: string) => clicked.push(move) },
    });

    // Click the first PV move button in the last column
  ...
  test("renders table header columns", async ({ mount }) => {
    ...
    await expect(component).toBeVisible();
    // All header columns should be present
  ...
  test("has accessible role=region attribute", async ({ mount }) => {
    ...
    // role="region" is on the component root element itself
  ...
  test("renders nodes and NPS as formatted numbers", async ({ mount }) => {
    ...
    await expect(component).toBeVisible();
    // Formatted numbers should appear (exact format depends on formatNumber helper)
