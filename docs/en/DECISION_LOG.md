# Decision Log (DECISION_LOG.md)

This document is an index to the ADRs (Architecture Decision Records) that record the major design decisions of the project. Please refer to individual files for details.

> **Note (2026-02-19)**: ADR-003 to ADR-013 were assigned during the early design phase but were absorbed or superseded by later integrations (e.g., ADR-014 to ADR-026). These numbers are currently skipped to maintain historical integrity.

---

## 🏗️ Architecture

- [ADR-001: Adoption of Monorepo Structure](./adr/001-monorepo-structure.md) (Accepted - 2026-01-28)
- [ADR-002: Streaming Communication via Async Iterable](./adr/002-async-iterable.md) (Accepted - 2026-01-29)
- [ADR-014: Physical Separation of Core and Adapters](./adr/014-mit-license-architecture.md) (Accepted - 2026-02-10)
- [ADR-031: Strict TypeScript Monorepo and Project References](./adr/031-strict-typescript-monorepo.md) (Accepted - 2026-02-18)
- [ADR-032: Migration to 2026 Q1 Tech Stack (Zenith Tier)](./adr/032-zenith-tech-stack-2026.md) (Accepted - 2026-02-18)
- [ADR-040: Ensemble Adapter Architecture](./adr/040-ensemble-adapter-architecture.md) (Proposed - 2026-02-21)
- [ADR-046: Standardization of Directory Structures and Naming Conventions](./adr/046-structure-standardization.md) (Completed - 2026-02-23)

## 📦 Resource Management & Security

- [ADR-019: Centralization of Resource Management via EngineLoader (Mandatory SRI)](./adr/019-engine-loader-centralization.md) (Accepted - 2026-02-13)
- [ADR-038: Privacy-First Logging Policy](./adr/038-privacy-first-logging.md) (Accepted - 2026-02-20)
- [ADR-039: OPFS Storage Implementation](./adr/039-opfs-storage-implementation.md) (Completed - 2026-02-21)
- [ADR-043: ResourceInjector Handshake Protocol](./adr/043-resource-injector-handshake.md) (Completed - 2026-02-21)

## 🎨 UI & Presentation

- [ADR-027: Framework-Agnostic UI and Reactive Core](./adr/027-framework-agnostic-ui.md) (Accepted - 2026-02-17)
- [ADR-029: Definition of Zenith Tier Quality Standards](./adr/029-zenith-tier-quality-standards.md) (Accepted - 2026-02-18)
- [ADR-034: UI Modular Split and Monitor Separation](./adr/034-ui-modular-split-and-monitor-separation.md) (Accepted - 2026-02-19)
