...
5.  **視覚的順序に基づいた ARIA 座標**:
    - 盤面が反転（isFlipped / orientation="black"）している場合、スクリーンリーダー向けの読み上げラベル（「◯行 ◯列」等）は、配列のインデックスではなく、実際に画面に表示されている視覚的な位置関係から算出された座標を反映させなければなりません。
6.  **複雑なプロパティの Ref による代入 (React)**:
    - `pieceNames` のようなオブジェクト型のプロパティを React からカスタム要素に渡す際、属性（Attribute）としての文字列化を避け、`useRef` と `useEffect` を用いて DOM 要素のプロパティに直接代入します。これにより、シリアライズによるオーバーヘッドと型情報の欠落を防止します。
