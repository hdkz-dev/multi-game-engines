...
## 補足: リソースの無効化 (Invalidation)

`EngineBridge.unregisterAdapter()` によってアダプターが明示的に破棄された場合、そのアダプターを参照している既存の `EngineFacade` インスタンスは「無効化」された状態になります。以降の操作（`search()` 等）は、基礎となる WebWorker が既に終了しているため、`INTERNAL_ERROR` または通信エラーとして失敗します。利用者は必要に応じて `EngineFacade.onStatusChange` を購読し、エンジンの消失を検知する必要があります。
