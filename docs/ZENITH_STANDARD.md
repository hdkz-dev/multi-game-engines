...
## 🛡️ 6. 堅牢性・レジリエンス基準 (Robustness & Resilience)

### 6.1. Empirical 98% Line Coverage

- **基準**: `core` パッケージにおけるラインカバレッジを 98.4% 以上に保つ。
- **現状 (2026-05-11 計測, PR #140〜#161 マージ後)**: lines **98.45%** / statements **98.01%** / branches **89.05%** / functions **94.4%** ✅ **目標達成**。2026-05-09 に 84.6% へ低下したが、PR #140〜#161 で 13.85 pt 取り戻し、当初目標 98.4% を +0.05 pt 上回って Coverage Restoration バックログをクローズ。CI は `lines ≥98.4 / branches ≥88` でしきい値固定 (PR #161)。
...
- **基準**: オプショナルなミドルウェア（テレメトリ、ロギング等）の故障が、エンジンのコア機能（探索）を停止させないこと。
- **実装**: `EngineFacade` のミドルウェアフックを `try-catch` で絶縁し、エラー発生時はエンジンの状態を維持したまま `EngineError` として外部に通知する。

### 6.3. Message Integrity (Stream Buffering)

- **基準**: ネットワークパケットや OS パイプの分割が発生しても、メッセージの整合性を保証すること。
- **実装**: `NativeCommunicator` 等において動的バッファリングを実装し、巨大な PV 文字列などが分割されても、完全な 1 行として再構築してからパースする。

### 6.4. Circular Reference & Overflow Protection

- **基準**: プロトコルデータや設定における循環参照によるスタックオーバーフローを物理的に防止すること。
- **実装**: `ProtocolValidator` に `WeakSet` による循環検知ロジックを組み込み、悪意あるネストされた入力からプロセスを保護する。
