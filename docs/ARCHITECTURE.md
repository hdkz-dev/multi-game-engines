...
1.  **Reactive Core (`ui-core`)**:
    ...
    - **機能**: エンジンからの高頻度な `info` ストリームの正規化（Zod によるランタイム検証）、状態管理、および `requestAnimationFrame` (RAF) に同期した描画スケジューリング。
    - **利点**: 万が一メインスレッドが重くなっても、RAF スケジューリングにより UI のカクつき（Jank）を最小限に抑えます。
