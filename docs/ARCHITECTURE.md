...
- **キーボード中心の盤面操作**: `ui-elements` 系の盤面は矢印キーによる相対移動だけでなく、`Home` / `End`、`Ctrl+Home` / `Ctrl+End`、`PageUp` / `PageDown` による行端・盤面端・列維持ジャンプを備え、キーボードのみでも高速に局面を走査できます。
...
- **型解決の局所安定化**: モノレポ内の UI ハブパッケージは、依存する Web Components パッケージに対して `tsconfig.paths` を明示し、並列 `build` / `typecheck` 実行時でもエクスポート型の解決が順序依存にならないようにします。
