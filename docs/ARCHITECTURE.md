...
1. **純粋なコア (Pure Core & Pay-as-you-go)**:
   ...
   - **ドメイン隔離**: 特定のゲーム（チェス、将棋等）に依存するロジックは `/chess`, `/shogi` などのサブパスに物理的に隔離されています。
   - 利用者は、使用するドメインのみをインポートすることで、不要なコード、型定義、アセット（駒のSVG等）がバンドルされるのを防ぎます。
...
6. **構造化スコア (Structured Score)**: チェス (cp, mate)、将棋 (points)、囲碁 (winrate) 等、異なるゲームの評価特性を `{ cp, mate, points, winrate }` の統一オブジェクトに一般化し、UI レイヤーでの抽象的な分析表示を可能にします。
7. **Zenith Quality (Zero-Any Architecture)**: プロダクションコードにおける `any` の 100% 排除と、`exactOptionalPropertyTypes` 等の極めて厳格な TypeScript 設定により、実行時の予期せぬエラーを構造的に防止します。
...
- **アンサンブル・アダプター (Swarm Architecture)**: 複数のエンジンを束ね、単一の `IEngine` として振る舞わせるメタ・アダプターを提供します。多数決 (Majority Vote) や評価値ベース (Best Score) などの戦略を用い、複数の知性を統合した「Swarm（群知能）」的な意思決定を可能にします。
