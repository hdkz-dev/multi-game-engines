...
| オープン PR / Issue                  | **0件 / 0件**                                                                                                             |
| オープン Dependabot alerts           | **0件** ✅ (2026-07-21 に再発した4件を PR #227 で解消済み)                                                                |
| `pnpm audit`                         | **0 vulnerabilities** ✅ prod / dev とも (2026-07-21 確認)                                                                |
...
## ✅ 直近完了タスク (2026年7月21日, セキュリティ) — 再発した Dependabot alerts 4件の解消と override の再発防止

2026-07-18 時点で 0 件だった alerts が 4 件再発。PR #227 でバージョンを引き上げて解消した後、**再発の根本原因である指定方式**を後続 PR で是正した。

### 解消した 4 件 (PR #227)

| パッケージ        | scope | severity | patched |
| ----------------- | ----- | -------- | ------- |
| `brace-expansion` | dev   | high     | 5.0.7   |
| `js-yaml`         | dev   | high     | 4.3.0   |
| `shell-quote`     | prod  | high     | 1.9.0   |
| `protobufjs`      | prod  | medium   | 8.6.6   |

### 根本原因と再発防止

`pnpm.overrides` が**下限固定 (`">=X.Y.Z"`) / 完全固定**で書かれており、上流が新しい patch を出すたびに陳腐化する。実際、今回の 3 件はいずれも既存 override が patched version より古くなって再発したもの。

対策として **メジャー内の上限付きレンジ** (`">=X.Y.Z <次メジャー"`) に統一。上流の patch を自動追従しつつ、意図しないメジャー跨ぎは防ぐ。

> 検証中、無スコープの `">=X"` は pnpm が最新を選ぶためメジャーを跨ぐ (undici 7→8 / js-yaml 4→5) ことを確認したため、上限指定を必須とした。

### 検証結果

| 項目                    | 結果                                |
| ----------------------- | ----------------------------------- |
| `pnpm audit` (prod/dev) | **0 vulnerabilities** ✅            |
| オープン alerts         | **0件** ✅                          |
| `pnpm typecheck`        | 99 tasks 全通過 ✅                  |
...
- ESLint 10.4.1、vitest 4.1.7、nuxt 4.4.6、vue-tsc 3.3.1、typescript-eslint 8.59.4、@eslint-react/eslint-plugin 5.8.3、vite 8.0.14、postcss 8.5.15、wrangler 等 18 依存のパッチ／マイナー更新を取り込み予定。
