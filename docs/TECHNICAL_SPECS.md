...
### 7.1 仮想エンジン (Mocking)
テスト環境や UI プロトタイピングのために、ネットワーク通信を行わずに擬似的な応答を返す `MockAdapter` を提供します。

### 7.2 厳格なエラー型定義
`L4EngineError` クラスを定義し、エラーコード（NETWORK_ERROR, WASM_INIT_ERROR, PROTOCOL_ERROR 等）を明確に分けることで、プログラムによる適切な例外処理を可能にします。
